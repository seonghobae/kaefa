...
aefa <- efa <- function(data, model = NULL, minExtraction = 1, maxExtraction = if (is.data.frame(data)) if (ncol(data) <
    ...
    BURNIN = 1500, SEMCYCLES = 1000, covdata = NULL, fixed = .covdataFixedEffectComb(covdata), random = lapply(c(.covdataClassifieder(covdata)$random, 'items'), FUN = function(X){eval(parse(text = paste0('as.formula(',paste0('~1|',X), ')')))}), key = NULL, accelerate = "squarem", symmetric = F, saveModelHistory = T,
