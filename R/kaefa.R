...
estIRT <- function(data, model = 1, GCEvms = NULL, GenRandomPars = T, NCYCLES = 4000, BURNIN = 1500,
  ...
  if (is.numeric(model)) {
    if (model == 1) {
      ...
      if (max(psych::describe(data)$range) != 1) {
        ...
        if (!is.null(key)) {
          # with key
          estItemtype <- c("4PLNRM", "3PLNRM", "3PLNRMu", "2PLNRM", "nominal", "gpcm", "gpcmIRT",
                           "graded", "grsm", "grsmIRT", "Rasch", "rsm")
        } else {
          # without key
          estItemtype <- c("nominal", "gpcm", "gpcmIRT", "graded", "grsm", "grsmIRT", "Rasch",
                           "rsm")
        }
      } else {
        # dich UIRT
      ...
      if (max(psych::describe(data)$range) != 1) {
        ...
        if (!is.null(key)) {
          # poly MIRT with key
          estItemtype <- c("4PLNRM", "3PLNRM", "3PLNRMu", "2PLNRM", "nominal", "gpcm", "graded",
                           "grsm")
        } else {
    ...
    for (i in 1:length(randomEffectCandidates)) {
      for (j in estItemtype) {
        if (!is.null(key) && sum(c("4PLNRM", "3PLNRM", "3PLNRMu", "2PLNRM") %in% j) > 0) {
          k <- k + 1
          modConditional[[k]] %<-% try(fitMLIRT(accelerate = accelerate, data = mirt::key2binary(data,
                                                                                                 key), model = model, itemtype = if (j == "4PLNRM")
                                                                                                   "4PL" else if (j == "3PLNRM")
                                                                                                     "3PL" else if (j == "3PLuNRM")
                                                                                                       "3PLu" else if (j == "2PLNRM")
                                                                                                         "2PL" else j, GenRandomPars = GenRandomPars, NCYCLES = NCYCLES, BURNIN = BURNIN, SEMCYCLES = SEMCYCLES,
                                                symmetric = symmetric, covdata = covdata, fixed = fixed, random = eval(parse(text = randomEffectCandidates[i]))))
        } else {
          k <- k + 1
          modConditional[[k]] %<-% try(fitMLIRT(accelerate = accelerate, data = data, model = model,
                                                itemtype = j, GenRandomPars = GenRandomPars, NCYCLES = NCYCLES, BURNIN = BURNIN,
  ...
  for (j in estItemtype) {
    l <- l + 1
    modUnConditional[[l]] %<-% try(mirt::mirt(data = data, model = model, method = "MHRM", itemtype = j,
                                              accelerate = accelerate, SE = T, GenRandomPars = GenRandomPars, key = key, calcNull = T,
                                              technical = list(NCYCLES = NCYCLES, BURNIN = BURNIN, SEMCYCLES = SEMCYCLES, symmetric = symmetric)))
  }
...
exploratoryIRT <- function(data, minExtraction = 1, maxExtraction = if (ncol(data) < 10) ncol(data) else 10,
  ...
  for (i in minExtraction:maxExtraction) {
    estModels[[i]] %<-% estIRT(data = data, model = i, GCEvms = GCEvms, GenRandomPars = GenRandomPars,
                               NCYCLES = NCYCLES, BURNIN = BURNIN, SEMCYCLES = SEMCYCLES, covdata = covdata, fixed = fixed,
...
#'
#' }
aefa <- function(data, model = NULL, minExtraction = 1, maxExtraction = if (ncol(data) < 10) ncol(data) else 10,
                 GCEvms = NULL, GenRandomPars = T, NCYCLES = 4000, BURNIN = 1500, SEMCYCLES = 1000, covdata = NULL,
  ...
  while (!STOP) {
    ...
    if (is.null(model) && class(data) == "data.frame") {
      # run exploratory IRT
      try(estModel <- exploratoryIRT(data = data[!colnames(data) %in% badItemNames], minExtraction = minExtraction,
                                     maxExtraction = maxExtraction, GCEvms = GCEvms, GenRandomPars = GenRandomPars, NCYCLES = NCYCLES,
                                     BURNIN = BURNIN, SEMCYCLES = SEMCYCLES, covdata = covdata, fixed = fixed, random = random,
                                     key = key, accelerate = accelerate, symmetric = symmetric))

    } else if (attr(class(model), "class") == "mirt.model" && class(data) == "data.frame") {
      # run confirmatory IRT
      try(estModel <- estIRT(data = data, model = model, GCEvms = GCEvms, GenRandomPars = GenRandomPars,
                             NCYCLES = NCYCLES, BURNIN = BURNIN, SEMCYCLES = SEMCYCLES, covdata = covdata, fixed = fixed,
                             random = random, key = key, accelerate = accelerate, symmetric = symmetric))
    }
    ...
    if (exists("estModel")) {
      ...
      if (sum(colnames(estItemFit) %in% "Zh")) {
        if (sum(estItemFit$Zh < -1.96)) {
          badItemNames <- c(badItemNames, estItemFit$item[which(estItemFit$Zh == min(estItemFit$Zh,
                                                                                     na.rm = T))])
        } else {
          STOP <- T
        }
      ...
      if (attr(class(model), "class") == "mirt.model" && class(data) == "data.frame") {
        for (i in 1:NROW(model)) {
          model$x[i, 2] <- eval(parse(text = paste0("c(", gsub("-", ":", model$x[i, 2]), ")")))[!eval(parse(text = paste0("c(",
                                                                                                                          gsub("-", ":", model$x[i, 2]), ")"))) %in% estItemFit$item[which(estItemFit$Zh ==
