...
aefaInit <- function(GCEvms = NULL, debug = F) {
    ...
    if (is.null(suppressWarnings(NCmisc::top()$CPU$idle))) {
        ...
        parallelProcessors <- round(parallel::detectCores(all.tests = FALSE, logical = FALSE)/4)
        if (2 >= parallelProcessors) {
            parallelProcessors <- 2
        }
    ...
    if (!is.null(GCEvms)) {
        ...
        if (require("Rmpi")) {
            options(aefaConn = future::plan(future::cluster, workers = parallel::makeCluster(spec = if (is.null(GCEvms)) 
                GCEvms else parallelProcessors, type = "MPI")), gc = T)
            ...
            options(aefaConn = (try(future::plan(strategy = list(future::tweak(future::cluster(workers = parallelProcessors)), 
                future::multiprocess(workers = parallelProcessors)), gc = T), silent = T)))
        }
...
fitMLIRT <- function(data = data, model = model, itemtype = NULL, accelerate = accelerate, GenRandomPars = GenRandomPars, NCYCLES = NCYCLES, 
    ...
    modMLIRT_latentLevel <- mirt::mixedmirt(data = data, model = model, accelerate = accelerate, itemtype = itemtype, SE = T, 
...
evaluateItemFit <- function(mirtModel, GCEvms = NULL, rotate = "bifactorQ") {
    ...
    if (class(mirtModel)[1] == "MixedClass") {
        ...
        mirtModel <- mirt::mirt(data = mirtModel@Data$data, model = mirtModel@Model$model, itemtype = mirtModel@Model$itemtype, 
            pars = modMLM, method = "QMCEM", SE = F, calcNull = T)
    }
    ...
    if (attr(class(mirtModel), "package") == "mirt") {
        ...
        itemFitList <- c("modFit_Zh", "modFit_SX2", "modFit_PVQ1", "modFit_infit")[c(exists("modFit_Zh"), exists("modFit_SX2"), 
            exists("modFit_PVQ1"), exists("modFit_infit"))]
    ...
    if (is.numeric(model)) {
        if (model == 1) {
            ...
            if (max(psych::describe(data)$range) != 1) {
                ...
                if (!is.null(key)) {
                  # with key
                  estItemtype <- c("4PLNRM", "3PLNRM", "3PLNRMu", "2PLNRM", "nominal", "gpcm", "gpcmIRT", "graded", "grsm", 
                    "grsmIRT", "Rasch", "rsm")
                } else {
                  # without key
            ...
            for (j in estItemtype) {
                if (!is.null(key) && sum(c("4PLNRM", "3PLNRM", "3PLNRMu", "2PLNRM") %in% j) > 0) {
                  ...
                  if (sum(c("grsmIRT", "gpcmIRT", "spline", "rsm") %in% j) == 0) {
                    modConditional[[k]] %<-% try(fitMLIRT(accelerate = accelerate, data = data, model = model, itemtype = j, 
                      GenRandomPars = GenRandomPars, NCYCLES = NCYCLES, BURNIN = BURNIN, SEMCYCLES = SEMCYCLES, symmetric = symmetric, 
                      covdata = covdata, fixed = fixed, random = eval(parse(text = randomEffectCandidates[i]))))
                  } else {
        ...
        for (m in c("sandwich", "Oakes", "sandwich.Louis", "SEM")) {
            for (n in c(T, F)) {
                ...
                modDiscrete[[mm]] %<-% try(mirt::mdirt(data = data, model = model, SE = T, SE.type = m, accelerate = accelerate, 
                  SE = T, GenRandomPars = GenRandomPars, empiricalhist = n, calcNull = T, technical = list(NCYCLES = NCYCLES, 
        ...
        if ((is.data.frame(data) | is.matrix(data))) {
            ...
            for (i in 1:NROW(data)) {
                if (sum(c("MixedClass", "SingleGroupClass", "DiscreteClass") %in% class(data[[i]])) != 0) {
                  ...
                  estModel[[NROW(estModel) + 1]] <- try(exploratoryIRT(data = data.frame(data[[i]][, !colnames(data[[i]]) %in% 
                    ...
                    for (j in 1:nrow(model[[i]])) {
                      if (!model[[i]]$x[j, 1] %in% c("COV", "MEAN", "FREE", "NEXPLORE")) {
                        model[[i]]$x[j, 2] <- eval(parse(text = paste0("c(", gsub("-", ":", model[[i]]$x[j, 2]), ")")))[!eval(parse(text = paste0("c(", 
                          ...
                          na.rm = T))]]  ## FIXME # convert elements
                      }
                    }
                  }
                }
