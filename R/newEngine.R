...
#' }
  engineAEFA <- function(data, model = 1, GenRandomPars = T, NCYCLES = 4000, BURNIN = 1500, SEMCYCLES = 1000, covdata = NULL, fixed = ~1, random = list(), key = NULL, accelerate = "squarem",
    ...
    estModels %<-% {
      ...
      for(i in model){ # exploratory i th model
        ...
        exploratoryModels[[i]] %<-% {
          ...
          for(j in estItemtype){ # itemtype j for model i
            modUnConditional[[j]] %<-% {
            ...
            modConditional[[j]] %<-% {
              if (!is.null(covdata)) {
                ...
                for (k in 1:randomEffectCandidates){ # and
                        ...
                        "4PL" else if (j == "3PLNRM")
                          "3PL" else if (j == "3PLuNRM")
                            "3PLu" else if (j == "2PLNRM")
                              "2PL" else j, GenRandomPars = GenRandomPars, NCYCLES = NCYCLES, BURNIN = BURNIN, SEMCYCLES = SEMCYCLES, symmetric = symmetric, covdata = covdata, fixed = fixed, random = eval(parse(text = k))))
                    } else {
                      if (sum(c("grsmIRT", "gpcmIRT", "spline", "rsm") %in% j) == 0) {
                        try(fitMLIRT(accelerate = accelerate, data = data, model = model, itemtype = j, GenRandomPars = GenRandomPars, NCYCLES = NCYCLES, BURNIN = BURNIN, SEMCYCLES = SEMCYCLES,
                                     symmetric = symmetric, covdata = covdata, fixed = fixed, random = eval(parse(text = k))))
                      } else {
              ...
              for (m in c("sandwich", "Oakes")) {
                for (n in c(T, F)) {
                  modDiscreteTemp[[NROW(as.list(modDiscreteTemp))+1]] %<-% {
                    try(mirt::mdirt(data = data, model = i, SE = T, SE.type = m,
                                    accelerate = accelerate, GenRandomPars = GenRandomPars,
                                    empiricalhist = n, technical = list(NCYCLES = NCYCLES, BURNIN = BURNIN,
                                                                        SEMCYCLES = SEMCYCLES, symmetric = symmetric),
                                    covdata = covdata, formula = if (fixed == ~1) NULL else fixed))
                  }
                }
              }
      ...
      unlist(as.list(exploratoryModels))
    }
    unlist(as.list(estModels))
  }
